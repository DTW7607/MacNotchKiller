...
程序启动时会创建一个与内置 Retina 屏幕逻辑尺寸、实际像素和刷新率一致的虚拟显示器，但不会立即移动鼠标或显示透传窗口。程序持续监听焦点窗口 3 秒，将最后一个有效焦点窗口移入虚拟屏后，才在内置屏幕上显示无边框置顶透传窗口并接管键鼠。
...
- 虚拟屏准备完成后，在终端提示用户于 3 秒内将目标窗口置于焦点。
- 3 秒内持续记录最新焦点窗口；倒计时结束后将其移动到虚拟屏并进入 macOS 原生全屏。
- 移动完成后才在内置屏幕显示虚拟屏内容并接管鼠标。
- 不使用 ScreenCaptureKit，也不枚举或排除任何窗口；程序按虚拟显示器 ID 从 `CGDisplayStream` 回调直接取得 WindowServer 的 `IOSurface`，并用 Metal 映射到内置屏透传窗口。
